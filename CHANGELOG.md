...
- New light-only tokens in `design-tokens.json`: `surface.raised-light`, `line.strong-light`, `elevation.1-light`, `elevation.2-light`. New `mode.{default,prezzo}` namespace documenting the orthogonal mode axis. `$version` bumped 0.3.9 → 0.9.0 to lead `package.json`.
