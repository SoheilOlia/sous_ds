...
- `✓` Sanctioned attention carriers: `LiveDot`, `InlineStatus` in `tone="live"`, a live-pill prefix, a live-tone toast marker, a sparse `DottedChart` point marking anomaly/attention, a `DotTimeline` column in `live` state, or the head dot of `PulseTrail`. (`LiveBlock` is also a live indicator, but its chasing motion carries the signal — it does not paint in `accent-live`.)
...
### Block (multi-cell live indicator)

`<LiveBlock>` is the sibling of `<LiveDot>` for surfaces that want a multi-cell, motion-led indicator instead of a single flat dot. Inline 12px footprint carrying the same chasing-blocks animation as `<BlockLoader>` (seven blocks chasing an eight-position 3×3 path on a 10-second loop). Blocks paint in `--ds-text-primary`, not `--ds-accent-live`: `<LiveDot>` earns the red because a single static dot needs hue to carry "live", but here the chasing motion already carries the live signal, so color stays neutral. No extra opacity pulse on top — that would compete with the chase and read as noise. Default cell 3px / gap 1px gives a 12px outer, tunable via `--ds-liveblock-*` custom properties. Same inline-flex row layout, same `labels` / `labelStep` / `labelHold` props, same delegation to `rotateLabels()` from the motion primitive — drop-in swap for `<LiveDot labels={...}>`. Use `<LiveDot>` as the default; reach for `<LiveBlock>` when the surface warrants a multi-element, kinetic read (e.g., next to the `<ThinkingCube>` vocabulary, or in toolbars where a moving cluster reads stronger than a single dot).
...
- **`<BlockLoader>`** — seven white blocks chasing each other around an eight-position path on a 3×3 grid (top-left → top-mid → top-right → mid-right → CENTER → bottom-mid → bottom-left → mid-left → loop). Every block runs the identical keyframe with a per-block stagger (negative delay = −(cycle / 7) per block) so the seven of them form an evenly-spaced trail at any given frame. Default cycle is ten seconds, `infinite` (exempt from the 300ms ceiling per the system's continuous-motion rule). Axis-aligned silhouette. `transform: translate()` keyframes — GPU-accelerated. Grayscale only: blocks paint in `--ds-text-primary`. Default cell 12px gives a 48px outer — compact enough to sit inline with mono body copy or in a toolbar slot; tunable via `--ds-block-loader-*` custom properties. `prefers-reduced-motion` parks each block at its home cell so the composed shape still reads, minus motion. The same chasing animation is reused at 12px footprint inside `<LiveBlock>` (recolored in `accent-live`) — the loader and the live indicator share the same motion vocabulary so the system reads as one family.
