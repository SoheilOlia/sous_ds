...
- **`<ThinkingCube>`** — single 16×16 CSS 3D cube paired with a rotating mono label. Canonical agent-thinking indicator: the cube ticks through discrete 90° face-flips on a 2000ms loop (stepped keyframes, not smooth spin) while the label types through `["AGENTING", "WORKING", "THINKING", "REASONING"]` via `rotateLabels()` from the motion primitive. Cube stays at a low iso tilt (`-8deg` X, `-12deg` Y base) so it reads as a *cube*, not a diamond — different silhouette than `<BoxLoader>`. Grayscale face shading matches the rest of the system (top=`text-primary`, front=`text-secondary`, side/back/bottom=`text-muted`). Inline-flex row; drops into any body-copy line where a live indicator belongs. Replaces the prior red-dot `<LiveDot labels={...}>` pattern in agent-thinking contexts — the dot was a binary on/off signal; the cube adds dimensionality without taking more space.
