...
### Font contract
- Primary face: `Cash Sans` (via `--ds-font-sans`)
- Mono face: `Cash Sans Mono` (via `--ds-font-mono`)
- Forbidden as primary: Inter, Roboto, Helvetica, Arial, system-ui, Open Sans, Lato, Poppins
- Tracking: display `-0.03em`, headings `-0.015em` to `-0.02em`. Tight tracking at display sizes reinforces the "precision instrument" stance.
